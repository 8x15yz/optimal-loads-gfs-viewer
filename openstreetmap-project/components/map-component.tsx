...
export default function MapComponent() {
  ...
  const loadWindData = async () => {
    ...
    try {
      ...
      console.error("바람 데이터 로드 실패:", error)
      alert(`바람 데이터를 로드할 수 없습니다: ${error}`)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="absolute top-4 left-4 z-[1000] bg-white rounded-lg shadow-lg p-4 min-w-[250px]">
        <h2 className="text-lg font-semibold mb-3">GFS 바람 방향 데이터</h2>
        ...
        {windData && (
          <div className="mt-3 pt-3 border-t border-gray-200">
            <div className="text-xs text-gray-600 space-y-1">
              <div>타임스탬프: {new Date(windData.timestamp).toLocaleString()}</div>
              <div>유효 데이터: {getValidDataCount()}개</div>
              <div>전체 격자: {windData.data.length}개</div>
              <div>
                해상도: {windData.resolution[0]}° × {windData.resolution[1]}°
              </div>
              <div>
                영역: {windData.bbox[0]}°-{windData.bbox[2]}°E, {windData.bbox[1]}°-{windData.bbox[3]}°N
              </div>
            </div>
          </div>
        )}
      ...
      {windData && (
        <div className="absolute top-4 right-4 z-[1000] bg-white rounded-lg shadow-lg p-3">
          <h3 className="text-sm font-semibold mb-2">바람 방향</h3>
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <div className="w-4 h-2 bg-blue-500"></div>
              <span className="text-xs">북풍 (0°-90°)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-4 h-2 bg-green-500"></div>
              <span className="text-xs">동풍 (90°-180°)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-4 h-2 bg-yellow-500"></div>
              <span className="text-xs">남풍 (180°-270°)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-4 h-2 bg-red-500"></div>
              <span className="text-xs">서풍 (270°-360°)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-4 h-2 bg-gray-400"></div>
              <span className="text-xs">결측값</span>
            </div>
          </div>
        </div>
      )}
