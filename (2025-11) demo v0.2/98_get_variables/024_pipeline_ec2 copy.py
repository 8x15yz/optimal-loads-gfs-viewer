...
PRODUCT_CODE = "024"
VARS = ["uo", "vo"]                    # 동/북향 해류성분
BUCKET = "optimal-loads"
S3_PREFIX_ROOT = f"cmems/{PRODUCT_CODE}"   # s3://optimal-loads/cmems/024/<var>/<YYYY>/<MM>/
...
def save_metadata(source: str, dataset_code: str, var: str, t: datetime, filename: str, s3_key: str):
    """S3 업로드 성공 후 MongoDB에 메타데이터 저장"""
    size_bytes = os.path.getsize(filename)
    doc = {
        "source": source,                    # "cmems"
        "dataset_code": dataset_code,        # "024"
        "variable": var,                     # "uo" | "vo"
        ...
        # 해상도: 0.083333 근사(고정). 필요시 계산 로직으로 교체 가능.
    ...
    while t <= END_UTC:
        for var in VARS:
            ...
            try:
                ...
                # ✅ 업로드 성공 직후 메타데이터 저장 (파일 삭제 전에)
                save_metadata("cmems", PRODUCT_CODE, var, t, out, s3_key)
