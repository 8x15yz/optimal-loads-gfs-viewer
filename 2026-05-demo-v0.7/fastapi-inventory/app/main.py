...
def _build_api_example(doc: Dict[str, Any], lat: float = 35.0, lon: float = 129.0, buffer_km: float = 50.0) -> str:
    """
    Final griddata API example - Center point + buffer method
    - Dynamically generated by fetching all fields from doc
    - datetime in RFC3339 as-is (NO URL encoding)
    """
...
def _build_api_example_corners(doc: Dict[str, Any], lat: float = 35.0, lon: float = 129.0, buffer_km: float = 50.0) -> str:
    """
    Final griddata API example - Northwest/Southeast corner method
    - Converts center point (lat, lon) and buffer (buffer_km) to corner coordinates
    - Uses nw_lon, nw_lat, se_lon, se_lat parameters
    """
    source = doc.get("source", "")
    dataset_code = doc.get("dataset_code", "")
    model = doc.get("model", "")
